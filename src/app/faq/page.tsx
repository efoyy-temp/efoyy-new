...
export default function FAQPage() {
  ...
  const getQuestions = (category: string): { question: string; answer: string }[] => {
  ...
  const filterQuestions = (questions: { question: string; answer: string }[]): { question: string; answer: string }[] => {
  ...
  const counts = Object.fromEntries(
    categories.map((cat) => [cat.key, filterQuestions(getQuestions(cat.key)).length])
  );
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Tabs defaultValue="general">
            <TabsList className="grid grid-cols-2 md:grid-cols-5 mb-8">
              {categories.map((cat) => (
                <TabsTrigger value={cat.key} className="flex items-center gap-2" key={cat.key}>
                  <cat.icon className="h-4 w-4" />
                  <span>{cat.label} ({counts[cat.key]})</span>
              </TabsTrigger>
              ))}
            </TabsList>
            ...
            {categories.map((cat) => (
              <TabsContent value={cat.key} key={cat.key}>
                <h2 className="text-2xl font-bold mb-4">{t(`${cat.key}Title`)}</h2>
                {filterQuestions(getQuestions(cat.key)).length > 0 ? (
                <Accordion type="single" collapsible className="w-full">
                    {filterQuestions(getQuestions(cat.key)).map((item: { question: string; answer: string }, index: number) => (
                      <AccordionItem key={index} value={`${cat.key}-item-${index}`}>
